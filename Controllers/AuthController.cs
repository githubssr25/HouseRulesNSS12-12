...
public class AuthController : ControllerBase
{
    ...
    public IActionResult Login([FromHeader(Name = "Authorization")] string authHeader)
    {
        try
        {
            ...
            // Log the hashed password and the input password
Console.WriteLine($"Stored PasswordHash (user.PasswordHash): {user.PasswordHash}");
Console.WriteLine($"Entered Password (plaintext): {password}");



// Optionally hash the entered password for comparison (to verify if hashing is consistent)
var manualHash = hasher.HashPassword(user, password);
Console.WriteLine($"Manually Hashed Password: {manualHash}");
